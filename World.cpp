...
void World::RenderWorld(RenderSystem *render_system) {
    ...
    if (o == 0) {
        font = render_system->LoadFont("C:\\Windows\\Fonts\\arial.ttf", 32);
        o = 1;
    }
    ...
    const float textWidth = render_system->GetTextWidth(font.get(), text, scale);

    const float screenWidth = Doomsday::GetInstance().GetWindow()->GetWidth();

    const float x = screenWidth - textWidth - padding;
    const float y = padding;

    render_system->RenderText(font.get(), text, x, y, 1, 0, 0, scale);
